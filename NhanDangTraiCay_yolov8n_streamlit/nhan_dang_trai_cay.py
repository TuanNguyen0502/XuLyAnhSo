...
def load_model():
    try:
        model = YOLO('trai_cay.onnx', task='detect')
        return model
...
def process_image(image, confidence=0.5):
    ...
    if len(results) > 0 and len(results[0].boxes) > 0:
        ...
        for box, cls, conf in zip(boxes, clss, confs):
            annotator.box_label(box, label=f"{names[int(cls)]} {conf:.2f}", txt_color=(255, 0, 0), color=(255, 255, 255))
    ...
    if st.button("Detect Objects"):
        if st.session_state.model_loaded:
            with st.spinner("Processing image..."):
                # Process image
                result_image = process_image(image, confidence)
...
if st.session_state.model_loaded:
    ...
    st.sidebar.write(f"Model: best.onnx")
